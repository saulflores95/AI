...
hidden = tflearn.fully_connected(input_, n_units=1000, activation='sigmoid', weights_init=tnorm)
hidden2 = tflearn.fully_connected(hidden, n_units=1000, activation='sigmoid', weights_init=tnorm)
hidden3 = tflearn.fully_connected(hidden2, n_units=1000, activation='sigmoid', weights_init=tnorm)
...
plt.title('Ej.3 Configuracion 4')
plt.plot(X, Y, 'k--', label='real points')
